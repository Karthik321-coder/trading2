...
# Enhanced CSS with trading-specific styling - FIXED HTML ENTITIES
st.markdown("""
<style>
    .main-header {
        font-size: 3.5rem;
    ...
    if st.session_state.current_signals:
        ...
        with col2:
            ...
            <div class="trading-card">
                ...
                <p><strong>Target:</strong> ${signal['target_price']:.2f}</p>
                <p><strong>Stop Loss:</strong> ${signal['stop_loss']:.2f}</p>
                <p><strong>Risk/Reward:</strong> {((signal['target_price'] - signal['stop_loss']) / max(signal['stop_loss'], 1)):.2f}</p>
            </div>
            """, unsafe_allow_html=True)
    else:
